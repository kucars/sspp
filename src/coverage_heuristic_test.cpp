...
int main( int argc, char **  argv)
{
    ...
    gridStartPose.position.y = -10 ;//-25
    ...
    Pose start(0,0,40,DTOR(0.0));//0,5,1
    ...
    std::string collisionCheckModelPath = ros::package::getPath("component_test") + "/src/mesh/burj_arab_scaled.obj"; //bridge_translated, burj_arab_scaled
    std::string occlusionCullingModelName = "burj_arab_scaled_densed.pcd";//bridge_translated_densed, burj_arab_scaled_densed
    ...
    // 2- Generate Grid Samples (generate grid and filter it if you want using the sspp itself)
    ...
    ss << targetCov;
    cc <<regGridConRad;
    std::string file_loc = ros::package::getPath("sspp")+"/resources/"+cc.str()+"_"+ss.str()+"%path_newtests1to4_"+occlusionCullingModelName+"scaledGPU_NewIG_Dynamic_dsscpp_test.txt";
