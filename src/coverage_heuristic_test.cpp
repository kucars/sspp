...
int main( int argc, char **  argv)
{
    ...
    pcl::io::loadPCDFile<pcl::PointXYZ> (ros::package::getPath("component_test")+"/src/pcd/etihad_nowheels_nointernal_scaled2_newdensed.pcd", *originalCloudPtr);

    pcl::PointCloud<pcl::PointXYZ>::Ptr coveredCloudPtr(new pcl::PointCloud<pcl::PointXYZ>);
    OcclusionCullingGPU occlusionCulling(nh,"etihad_nowheels_nointernal_scaled2_newdensed.pcd");
    ...
    gridStartPose.position.x = -10;//-18
    gridStartPose.position.y = -11 ;//-25
    ...
    std::string collisionCheckModelPath = ros::package::getPath("component_test") + "/src/mesh/etihad_nowheels_nointernal_scaled2_new.obj";
    std::string occlusionCullingModelName = "etihad_nowheels_nointernal_scaled2_newdensed.pcd";
...
//    pathPlanner->connectNodes();
    pathPlanner->dynamicNodesGenerationAndConnection(gridStartPose,gridSize,3.0,1.0);
...
//        pathPlanner->printNodeList();
    ...
    ss << targetCov;
    cc <<regGridConRad;
    std::string file_loc = ros::package::getPath("sspp")+"/resources/"+cc.str()+"_"+ss.str()+"%path_newtests1to4_"+occlusionCullingModelName+"IGGPU_Dynamic_NewScaled.txt";
